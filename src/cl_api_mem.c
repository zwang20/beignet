...
clEnqueueCopyBuffer(cl_command_queue command_queue,
    ...
    do {
        ...
        err = cl_event_exec(e, (cl_command_queue_allow_bypass_submit(command_queue) && (e_status == CL_COMPLETE))
                               ? CL_SUBMITTED : CL_QUEUED, CL_FALSE);
        if (err != CL_SUCCESS) {
            break;
        }
...
clEnqueueMapImageByKernel(cl_command_queue command_queue,
    ...
    err = cl_mem_record_map_mem_for_kernel(mem, ptr, &mem_ptr, offset, 0, origin, region,
                                           image->tmp_ker_buf,
                                           (map_flags & (CL_MAP_WRITE | CL_MAP_WRITE_INVALIDATE_REGION)) ? 1 : 0);
    image->tmp_ker_buf = NULL;
