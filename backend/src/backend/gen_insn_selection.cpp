...
namespace gbe {
    ...
    uint32_t getGenCompare(ir::Opcode opcode, bool inverse = false) {
        ...
        switch (opcode) {
            case OP_LE:
                return (!inverse) ? GEN_CONDITIONAL_LE : GEN_CONDITIONAL_G;
            case OP_LT:
                return (!inverse) ? GEN_CONDITIONAL_L : GEN_CONDITIONAL_GE;
            case OP_GE:
                return (!inverse) ? GEN_CONDITIONAL_GE : GEN_CONDITIONAL_L;
            case OP_GT:
                return (!inverse) ? GEN_CONDITIONAL_G : GEN_CONDITIONAL_LE;
            case OP_EQ:
                return (!inverse) ? GEN_CONDITIONAL_EQ : GEN_CONDITIONAL_NEQ;
            case OP_NE:
                return (!inverse) ? GEN_CONDITIONAL_NEQ : GEN_CONDITIONAL_EQ;
            default:
                NOT_SUPPORTED;
                return 0u;
        };
...
#undef I64Shift
        ...
        /*! Atomic instruction */
        void ATOMIC(Reg dst, uint32_t function, uint32_t srcNum, Reg src0, Reg src1, Reg src2, GenRegister bti,
                    vector<GenRegister> temps);

        /*! AtomicA64 instruction */
        void ATOMICA64(Reg dst, uint32_t function, uint32_t srcNum, vector<GenRegister> src, GenRegister bti,
                       vector<GenRegister> temps);

        /*! Read 64 bits float/int array */
        void READ64(Reg addr, const GenRegister *dst, const GenRegister *tmp, uint32_t elemNum, const GenRegister bti,
                    bool native_long, vector<GenRegister> temps);

        /*! Write 64 bits float/int array */
        void WRITE64(Reg addr, const GenRegister *src, const GenRegister *tmp, uint32_t srcNum, GenRegister bti,
                     bool native_long, vector<GenRegister> temps);
        ...
        void
        UNTYPED_READ(Reg addr, const GenRegister *dst, uint32_t elemNum, GenRegister bti, vector<GenRegister> temps);

        /*! Untyped write (up to 4 elements) */
        void
        UNTYPED_WRITE(Reg addr, const GenRegister *src, uint32_t elemNum, GenRegister bti, vector<GenRegister> temps);
        ...
        void SAMPLE(GenRegister *dst, uint32_t dstNum, GenRegister *msgPayloads, uint32_t msgNum, uint32_t bti,
                    uint32_t sampler, bool isLD, bool isUniform);

        /*! Encode vme instructions */
        void VME(uint32_t bti, GenRegister *dst, GenRegister *payloadVal, uint32_t dstNum, uint32_t srcNum,
                 uint32_t msg_type, uint32_t vme_search_path_lut, uint32_t lut_sub);
        ...
        void
        STORE_PROFILING(uint32_t profilingType, uint32_t bti, GenRegister tmp0, GenRegister tmp1, GenRegister ts[5],
                        int tsNum);
        ...
        for (auto &block: blockList)
            for (auto &insn: block.insnList) {
                ...
                for (uint32_t srcID = 0; srcID < srcNum; ++srcID) {
                    ...
                    if (it != spilledRegs.end()
                        ...
                        if (family == ir::FAMILY_QWORD && poolOffset == 1) {
                            poolOffset += simdWidth /
                                          8; // qword register fill could not share the scratch read message payload register
                        }
                ...
                while (!regSet.empty()) {
                    ...
                    if (!regSlot.isTmpReg) {
                        ...
                        for (uint32_t i = 1; i < 1 + (ctx.reservedSpillRegs * 8) / ctx.getSimdWidth(); i++)
                            unspill->dst(i) = ctx.getSimdWidth() == 8 ?
                                              ...
                                              GenRegister::vec16(GEN_GENERAL_REGISTER_FILE, registerPool + (i - 1) * 2,
                                                                 0);
                    ...
                    src.nr = registerPool + regSlot.poolOffset + src.nr;
                    src.physical = 1;
         ...
         */
                for (uint32_t dstID = 0; dstID < dstNum; ++dstID) {
                    ...
                    if (it != spilledRegs.end()
                        ...
                        if (family == ir::FAMILY_QWORD && poolOffset == 1) {
                            poolOffset += simdWidth /
                                          8; // qword register spill could not share the scratch write message payload register
                        }
                ...
                while (!regSet.empty()) {
                    ...
                    // change nr/subnr, keep other register settings
                    dst.physical = 1;
                    dst.nr = registerPool + regSlot.poolOffset;
                    dst.subnr = 0;
    ...
    ir::Register
    Selection::Opaque::replaceSrc(SelectionInstruction *insn, uint32_t regID, ir::Type type, bool needMov) {
    ...
    ir::Register
    Selection::Opaque::replaceDst(SelectionInstruction *insn, uint32_t regID, ir::Type type, bool needMov) {
        SelectionBlock *block = insn->parent;
        uint32_t simdWidth;
...
#define SEL_REG(SIMD16, SIMD8, SIMD1) \
  if (ctx.sel->isScalarReg(reg) == true) \
    ...
    GenRegister Selection::Opaque::selReg(ir::Register reg, ir::Type type) const {
        ...
        switch (family) {
            case FAMILY_BOOL:
                SEL_REG(uw16grf, uw8grf, uw1grf);
                break;
            case FAMILY_WORD:
                SEL_REG(uw16grf, uw8grf, uw1grf);
                break;
            case FAMILY_BYTE:
                SEL_REG(ub16grf, ub8grf, ub1grf);
                break;
            case FAMILY_DWORD:
                SEL_REG(f16grf, f8grf, f1grf);
                break;
            case FAMILY_QWORD:
                if (!this->hasLongType()) {
...
#undef SEL_REG
    ...
    void
    Selection::Opaque::UNPACK_BYTE(const GenRegister *dst, const GenRegister src, uint32_t elemSize, uint32_t elemNum) {
    ...
    void
    Selection::Opaque::PACK_BYTE(const GenRegister dst, const GenRegister *src, uint32_t elemSize, uint32_t elemNum) {
    ...
    void
    Selection::Opaque::CALC_TIMESTAMP(GenRegister ts[5], int tsN, GenRegister tmp, uint32_t pointNum, uint32_t tsType) {
        SelectionInstruction *insn = NULL;
        if (!this->hasLongType()) {
...
#undef SET_SEL_DBGINFO
    ...
    GenRegister getRegisterFromImmediate(ir::Immediate imm, ir::Type type, bool negate = false) {
        ...
        switch (type) {
            case TYPE_U32:
                return GenRegister::immud(imm.getIntegerValue() * sign);
            case TYPE_S32:
                return GenRegister::immd(imm.getIntegerValue() * sign);
            case TYPE_FLOAT:
                return GenRegister::immf(imm.getFloatValue() * sign);
            case TYPE_U16:
                return GenRegister::immuw(imm.getIntegerValue() * sign);
            case TYPE_S16:
                return GenRegister::immw((int16_t) imm.getIntegerValue() * sign);
            case TYPE_U8:
                return GenRegister::immuw(imm.getIntegerValue() * sign);
            case TYPE_S8:
                return GenRegister::immw((int8_t) imm.getIntegerValue() * sign);
            case TYPE_DOUBLE:
                return GenRegister::immdf(imm.getDoubleValue() * sign);
            case TYPE_BOOL:
                return GenRegister::immw((imm.getIntegerValue() == 0) ? 0 : -1);  //return 0xffff when true
            case TYPE_HALF: {
        ...
        INLINE bool emitOne(Selection::Opaque &sel, const ir::UnaryInstruction &insn, bool &markChildren) const {
            ...
            switch (opcode) {
                ...
                case ir::OP_BFREV:
                    ...
                    break;
                case ir::OP_COS:
                    sel.MATH(dst, GEN_MATH_FUNCTION_COS, src);
                    break;
                case ir::OP_SIN:
                    sel.MATH(dst, GEN_MATH_FUNCTION_SIN, src);
                    break;
                case ir::OP_LOG:
                    sel.MATH(dst, GEN_MATH_FUNCTION_LOG, src);
                    break;
                case ir::OP_EXP:
                    sel.MATH(dst, GEN_MATH_FUNCTION_EXP, src);
                    break;
                case ir::OP_SQR:
                    sel.MATH(dst, GEN_MATH_FUNCTION_SQRT, src);
                    break;
                case ir::OP_RSQ:
                    sel.MATH(dst, GEN_MATH_FUNCTION_RSQ, src);
                    break;
                case ir::OP_RCP:
                    sel.MATH(dst, GEN_MATH_FUNCTION_INV, src);
                    break;
                case ir::OP_BSWAP: {
                ...
                case ir::OP_SIMD_ANY: {
                    ...
                    const GenRegister constOne = GenRegister::retype(sel.selReg(sel.reg(ir::FAMILY_DWORD)),
                                                                     GEN_TYPE_UD);
...
#undef DECL_NOT_IMPLEMENTED_ONE_TO_MANY
        ...
        INLINE bool emitOne(Selection::Opaque &sel, const ir::LoadImmInstruction &insn, bool &markChildren) const {
            ...
            switch (type) {
                case TYPE_BOOL:
                    ...
                    break;
                case TYPE_U32:
                    sel.MOV(dst, GenRegister::immud(imm.getIntegerValue()));
                    break;
                case TYPE_S32:
                    sel.MOV(dst, GenRegister::immd(imm.getIntegerValue()));
                    break;
                case TYPE_FLOAT:
                ...
                case TYPE_U16:
                    sel.MOV(dst, GenRegister::immuw(imm.getIntegerValue()));
                    break;
                case TYPE_S16:
                    sel.MOV(dst, GenRegister::immw(imm.getIntegerValue()));
                    break;
                case TYPE_U8:
                    sel.MOV(dst, GenRegister::immuw(imm.getIntegerValue()));
                    break;
                case TYPE_S8:
                    ...
                    break;
                case TYPE_DOUBLE:
                    sel.MOV(dst, GenRegister::immdf(imm.getDoubleValue()));
                    break;
                case TYPE_S64:
                    sel.LOAD_INT64_IMM(dst, GenRegister::immint64(imm.getIntegerValue()));
                    break;
                case TYPE_U64:
                    sel.LOAD_INT64_IMM(dst, GenRegister::immuint64(imm.getIntegerValue()));
                    break;
                default:
                    NOT_SUPPORTED;
            }
       ...
       * value is not uniform, just do things under predication or mask */
        ...
        void read64Legacy(Selection::Opaque &sel,
            ...
            if (sel.hasLongType()) {
                ...
                for (uint32_t valueID = 0; valueID < valueNum; ++valueID) {
                    tmp[valueID] = GenRegister::retype(sel.selReg(sel.reg(ir::FAMILY_QWORD), ir::TYPE_U64),
                                                       GEN_TYPE_UL);
                }
        ...
        void readByteAsDWord(Selection::Opaque &sel,
            ...
            if (elemSize == GEN_BYTE_SCATTER_WORD)
                sel.MOV(GenRegister::retype(dst, GEN_TYPE_UW),
                        GenRegister::unpacked_uw(tmpReg, isUniform, sel.isLongReg(tmpReg)));
        ...
        void emitUnalignedByteGather(Selection::Opaque &sel,
            ...
            if (valueNum > 1) {
                ...
                if (sel.getSlowByteGather())
                    ...
                    if (isUniform)
                        sel.curr.noMask = 1;
                    shootByteGatherMsg(sel, insn, sel.selReg(dst, ir::TYPE_U32), address, elemSize, isUniform,
                                       addrSpace);
                    sel.pop();
    ...
    DECL_PATTERN(BitCastInstruction) {
        INLINE bool emitOne(Selection::Opaque &sel, const ir::BitCastInstruction &insn, bool &markChildren) const {
            ...
            const bool isInt64 = (srcType == TYPE_S64 || srcType == TYPE_U64 || dstType == TYPE_S64 ||
                                  dstType == TYPE_U64);
            const int simdWidth = sel.curr.execWidth;
    ...
    DECL_PATTERN(ConvertInstruction) {
        ...
        INLINE void
        convertBetweenHalfFloat(Selection::Opaque &sel, const ir::ConvertInstruction &insn, bool &markChildren) const {
            using namespace ir;
            const Type dstType = insn.getDstType();
            const Type srcType = insn.getSrcType();
        ...
        INLINE void
        convert32bitsToSmall(Selection::Opaque &sel, const ir::ConvertInstruction &insn, bool &markChildren) const {
            using namespace ir;
            const Type dstType = insn.getDstType();
            const Type srcType = insn.getSrcType();
            const GenRegister dst = sel.selReg(insn.getDst(0), dstType);
        ...
        INLINE void
        convertI64To16bits(Selection::Opaque &sel, const ir::ConvertInstruction &insn, bool &markChildren) const {
            using namespace ir;
            const Type dstType = insn.getDstType();
            const Type srcType = insn.getSrcType();
            const GenRegister dst = sel.selReg(insn.getDst(0), dstType);
        ...
        INLINE void
        convertI64ToI8(Selection::Opaque &sel, const ir::ConvertInstruction &insn, bool &markChildren) const {
            using namespace ir;
            const Type dstType = insn.getDstType();
            const Type srcType = insn.getSrcType();
            const GenRegister dst = sel.selReg(insn.getDst(0), dstType);
        ...
        INLINE void
        convertI64ToI32(Selection::Opaque &sel, const ir::ConvertInstruction &insn, bool &markChildren) const {
            using namespace ir;
            const Type dstType = insn.getDstType();
            const Type srcType = insn.getSrcType();
            const GenRegister dst = sel.selReg(insn.getDst(0), dstType);
        ...
        INLINE void
        convertI64ToFloat(Selection::Opaque &sel, const ir::ConvertInstruction &insn, bool &markChildren) const {
            using namespace ir;
            const Type dstType = insn.getDstType();
            const Type srcType = insn.getSrcType();
            const GenRegister dst = sel.selReg(insn.getDst(0), dstType);
                ...
                if (dag->insn.getOpcode() == OP_AND ||
                    ...
                    if (lowerI64Reg(sel, dag0, src0, GEN_TYPE_UD) &&
                        ...
                        switch (dag->insn.getOpcode()) {
                            default:
                            case OP_AND:
                                sel.AND(GenRegister::retype(dst, GEN_TYPE_UD), src0, src1);
                                break;
                            case OP_OR:
                                sel.OR(GenRegister::retype(dst, GEN_TYPE_UD), src0, src1);
                                break;
                            case OP_XOR:
                                sel.XOR(GenRegister::retype(dst, GEN_TYPE_UD), src0, src1);
                                break;
                        }
        ...
        INLINE void
        convertSmallIntsToI64(Selection::Opaque &sel, const ir::ConvertInstruction &insn, bool &markChildren) const {
            using namespace ir;
            const Type dstType = insn.getDstType();
            const Type srcType = insn.getSrcType();
            const GenRegister dst = sel.selReg(insn.getDst(0), dstType);
        ...
        INLINE void
        convertFToI64(Selection::Opaque &sel, const ir::ConvertInstruction &insn, bool &markChildren) const {
            using namespace ir;
            const Type dstType = insn.getDstType();
            const Type srcType = insn.getSrcType();
            const GenRegister dst = sel.selReg(insn.getDst(0), dstType);
            ...
            if (sel.hasLongType() && sel.hasLongRegRestrict() &&
                srcType == ir::TYPE_FLOAT) { // typical bsw float->long case
                ...
                GenRegister tmp = GenRegister::retype(
                        sel.selReg(sel.reg(FAMILY_DWORD, sel.isScalarReg(insn.getSrc(0))), TYPE_U32), type);
                sel.push();
                if (sel.isScalarReg(insn.getSrc(0))) {
                    sel.curr.execWidth = 1;
                    sel.curr.predicate = GEN_PREDICATE_NONE;
        ...
        INLINE void convertBetweenFloatDouble(Selection::Opaque &sel, const ir::ConvertInstruction &insn,
            ...
            GBE_ASSERTM(sel.hasDoubleType(),
                        "Double precision not supported on this device (if this is a literal, use '1.0f' not '1.0')");
        ...
        INLINE void
        convertBetweenHalfDouble(Selection::Opaque &sel, const ir::ConvertInstruction &insn, bool &markChildren) const {
            using namespace ir;
            const Type dstType = insn.getDstType();
            const Type srcType = insn.getSrcType();
            const GenRegister dst = sel.selReg(insn.getDst(0), dstType);
        ...
        INLINE void
        convertHalfToSmallInts(Selection::Opaque &sel, const ir::ConvertInstruction &insn, bool &markChildren) const {
            using namespace ir;
            const Type dstType = insn.getDstType();
            const Type srcType = insn.getSrcType();
            const GenRegister dst = sel.selReg(insn.getDst(0), dstType);
         ...
         strided by a DWord on the destination. */
            ...
            if (dstFamily == FAMILY_BYTE) {
                const uint32_t type = dstType == TYPE_U8 ? GEN_TYPE_UB : GEN_TYPE_B;
                tmp = GenRegister::retype(sel.unpacked_ub(sel.reg(FAMILY_DWORD, sel.isScalarReg(insn.getSrc(0)))),
                                          type);
                sel.MOV(tmp, src);
            } else {
                const uint32_t type = dstType == TYPE_U16 ? GEN_TYPE_UW : GEN_TYPE_W;
                tmp = GenRegister::retype(sel.unpacked_uw(sel.reg(FAMILY_DWORD, sel.isScalarReg(insn.getSrc(0)))),
                                          type);
                sel.MOV(tmp, src);
            }
            sel.pop();
        ...
        INLINE void
        convertSmallIntsToHalf(Selection::Opaque &sel, const ir::ConvertInstruction &insn, bool &markChildren) const {
            using namespace ir;
            const Type dstType = insn.getDstType();
            const Type srcType = insn.getSrcType();
            const GenRegister dst = sel.selReg(insn.getDst(0), dstType);
         ...
         strided by a DWord on the destination. */
            GBE_ASSERTM(sel.hasHalfType(), "Half precision not supported on this device");
            GenRegister tmp = GenRegister::retype(
                    sel.unpacked_uw(sel.reg(FAMILY_DWORD, sel.isScalarReg(insn.getSrc(0)))), GEN_TYPE_HF);
            sel.push();
            if (sel.isScalarReg(insn.getSrc(0))) {
                sel.curr.execWidth = 1;
                sel.curr.predicate = GEN_PREDICATE_NONE;
        ...
        INLINE void
        convertDoubleToSmallInts(Selection::Opaque &sel, const ir::ConvertInstruction &insn, bool &markChildren) const {
            using namespace ir;
            const Type dstType = insn.getDstType();
            const Type srcType = insn.getSrcType();
            const GenRegister dst = sel.selReg(insn.getDst(0), dstType);
        ...
        INLINE void
        convertI64ToDouble(Selection::Opaque &sel, const ir::ConvertInstruction &insn, bool &markChildren) const {
            using namespace ir;
            const Type dstType = insn.getDstType();
            const Type srcType = insn.getSrcType();
            const GenRegister dst = sel.selReg(insn.getDst(0), dstType);
        ...
        INLINE void
        convertSmallIntsToDouble(Selection::Opaque &sel, const ir::ConvertInstruction &insn, bool &markChildren) const {
            using namespace ir;
            const Type dstType = insn.getDstType();
            const Type srcType = insn.getSrcType();
            const GenRegister dst = sel.selReg(insn.getDst(0), dstType);
     ...
     * register here. */
        ...
        INLINE bool emit(Selection::Opaque &sel, SelectionDAG &dag) const {
            ...
            if (AM == AM_DynamicBti || AM == AM_StaticBti) {
                if (AM == AM_DynamicBti) {
                    Register btiReg = insn.getBtiReg();
                    sel.ATOMIC(dst, genAtomicOp, msgPayload, address, src1, src2, sel.selReg(btiReg, type),
                               sel.getBTITemps(AM));
                } else {
                    unsigned SI = insn.getSurfaceIndex();
                    sel.ATOMIC(dst, genAtomicOp, msgPayload, address, src1, src2, GenRegister::immud(SI),
                               sel.getBTITemps(AM));
                }
            } else if (addrSpace == ir::MEM_LOCAL) {
                ...
                if (addrBytes == 8)
                    addrDW = convertU64ToU32(sel, address);
                sel.ATOMIC(dst, genAtomicOp, msgPayload, addrDW, src1, src2, GenRegister::immud(0xfe),
                           sel.getBTITemps(AM));
            } else if (addrSpace == ir::MEM_GENERIC) {
                Register localMask = generateLocalMask(sel, address);
                sel.push();
    ...
    DECL_PATTERN(LabelInstruction) {
        INLINE bool emitOne(Selection::Opaque &sel, const ir::LabelInstruction &insn, bool &markChildren) const {
            ...
            const uint32_t simdWidth = sel.ctx.getSimdWidth();
            GBE_ASSERTM(label < sel.ctx.getMaxLabel(),
                        "We reached the maximum label number which is reserved for barrier handling");
            sel.LABEL(label);
    ...
    DECL_PATTERN(SampleInstruction) {
        ...
        INLINE bool emitOne(Selection::Opaque &sel, const ir::SampleInstruction &insn, bool &markChildren) const {
            ...
            sel.SAMPLE(dst.data(), insn.getDstNum(), msgPayloads, msgLen, bti, sampler, insn.getSamplerOffset() != 0,
                       false);
            return true;
        }
    ...
    DECL_PATTERN(TypedWriteInstruction) {
        ...
        INLINE bool emitWithSimd16(Selection::Opaque &sel, const ir::TypedWriteInstruction &insn, bool &markChildren,
                                   const GenRegister &header) const {
            using namespace ir;
        ...
        INLINE bool emitWithSimd8(Selection::Opaque &sel, const ir::TypedWriteInstruction &insn, bool &markChildren,
                                  const GenRegister &header) const {
            using namespace ir;
            GenRegister msgs[9]; // (header + U + V + W + LOD + 4)
            msgs[0] = header;
     ...
     * Shared algorithm with simd shuffle */
        INLINE bool
        emitSGBroadcast(Selection::Opaque &sel, const ir::SubGroupInstruction &insn, SelectionDAG &dag) const {
            using namespace ir;
    ...
    DECL_PATTERN(MediaBlockReadInstruction) {
        bool emitOne(Selection::Opaque &sel, const ir::MediaBlockReadInstruction &insn, bool &markChildren) const {
            ...
            uint32_t blocksize = fixBlockSize(insn.getWidth(), insn.getHeight(), vec_size, typeSize, simdWidth,
                                              block_width);
                ...
                if (typeSize == 1) {
                    ...
                    for (uint32_t i = 0; i < vec_size; i++) {
                        ...
                        sel.MOV(sel.getOffsetReg(valuesVec[i], 0, 16),
                                sel.getOffsetReg(GenRegister::retype(tmpVec[0], GEN_TYPE_UB), 0, i * simdWidth + 8));
                    }
                } else
    ...
    DECL_PATTERN(MediaBlockWriteInstruction) {
        bool emitOne(Selection::Opaque &sel, const ir::MediaBlockWriteInstruction &insn, bool &markChildren) const {
            ...
            if (block_width < 64) {
                ...
                if (typeSize == 1) {
                    for (uint32_t i = 0; i < vec_size; i++) {
                        sel.MOV(sel.getOffsetReg(GenRegister::retype(tmpVec[0], GEN_TYPE_UB), 0, i * simdWidth),
                                valuesVec[i]);
                        sel.MOV(sel.getOffsetReg(GenRegister::retype(tmpVec[0], GEN_TYPE_UB), 0, i * simdWidth + 8),
                                sel.getOffsetReg(valuesVec[i], 0, 16));
                    }
