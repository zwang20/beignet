...
namespace gbe {
    ...
    bool SelBasicBlockOptimizer::CanBeReplaced(const ReplaceInfo *info, const SelectionInstruction &insn,
        ...
        if (features & SIOF_LOGICAL_SRCMOD)
            if ((insn.opcode == SEL_OP_AND || insn.opcode == SEL_OP_NOT || insn.opcode == SEL_OP_OR ||
        ...
        if (features & SIOF_OP_MOV_LONG_REG_RESTRICT && insn.opcode == SEL_OP_MOV) {
            ...
            if (dst.isint64() && !info->replacement.isint64() &&
                info->elements != CalculateElements(info->replacement, insn.state.execWidth))
                return false;
        }
        ...
        if (info->intermedia.type == var.type && info->intermedia.quarter == var.quarter &&
            ...
            uint32_t elements = CalculateElements(var,
                                                  insn.state.execWidth);  //considering width, hstrid, vstrid and execWidth
...
#define RESULT() switch(insn->opcode) \
    ...
    bool
    SelGlobalImmMovOpt::CanBeReplaced(const ReplaceInfo *info, SelectionInstruction &insn, const GenRegister &var) {
