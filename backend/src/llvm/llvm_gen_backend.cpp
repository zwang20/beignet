...
namespace gbe {
   ...
   */
    ...
    static INLINE ir::AddressSpace addressSpaceLLVMToGen(unsigned llvmMemSpace) {
        switch (llvmMemSpace) {
            case 0:
                return ir::MEM_PRIVATE;
            case 1:
                return ir::MEM_GLOBAL;
            case 2:
                return ir::MEM_CONSTANT;
            case 3:
                return ir::MEM_LOCAL;
            case 4:
                return ir::MEM_GENERIC;
        }
        GBE_ASSERT(false);
        return ir::MEM_GLOBAL;
    }
...
#define TYPESIZEVEC(TYPE, SZ)\
  ...
  TYPESIZE(TYPE,16,SZ)
    ...
    static uint32_t getTypeSize(Module *M, const ir::Unit &unit, std::string &name) {
        ...
        TYPESIZEVEC(char, 1)TYPESIZEVEC(unsigned char, 1)TYPESIZEVEC(short, 2)TYPESIZEVEC(unsigned short, 2)TYPESIZEVEC(
                half, 2)TYPESIZEVEC(int, 4)TYPESIZEVEC(unsigned int, 4)TYPESIZEVEC(float, 4)TYPESIZEVEC(double,
                                                                                                        8)TYPESIZEVEC(
                long, 8)TYPESIZEVEC(unsigned long, 8)
        else {
...
#undef TYPESIZE
   ...
   */
     ...
     */
        ...
        typedef map<Value *, SmallVector < Value * , 4>>::
        iterator PtrOrigMapIter;
    ...
    public:
      ...
      initializeLoopInfoPass(*PassRegistry::getPassRegistry());
        ...
        void
        findPointerEscape(Value *ptr, std::set<Value *> &mixedPtr, bool recordMixed, std::vector<Value *> &revisit);
...
#undef DECL_VISIT_FN
        ...
        // Emit subgroup instructions
        void
        emitBlockReadWriteMemInst(CallInst &I, CallSite &CS, bool isWrite, uint8_t vec_size, ir::Type = ir::TYPE_U32);

        void
        emitBlockReadWriteImageInst(CallInst &I, CallSite &CS, bool isWrite, uint8_t vec_size, ir::Type = ir::TYPE_U32);
        ...
        void emitUnalignedDQLoadStore(ir::Register ptr, Value *llvmValues, ir::AddressSpace addrSpace, ir::Register bti,
                                      bool isLoad, bool dwAligned, bool fixedBTI);
        ...
        void emitAtomicInstHelper(const ir::AtomicOps opcode, const ir::Type type, const ir::Register dst,
                                  llvm::Value *llvmPtr, const ir::Tuple payloadTuple);

    private:
        void setDebugInfo_CTX(
                llvm::Instruction *insn); // store the debug infomation in context for subsequently passing to Gen insn
        ...
        for (unsigned i = 0; i < workList.size(); i++) {
            ...
            for (Value::use_iterator iter = work->use_begin(); iter != work->use_end(); ++iter) {
                ...
                if (isa<LoadInst>(theUser) || isa<StoreInst>(theUser) || isa<CallInst>(theUser)) {
                    ...
                    BtiValueMap.insert(std::make_pair(newLocBti, ConstantInt::get(Type::getInt32Ty(ptr->getContext()),
                                                                                  BTI_PRIVATE)));
                    pointerBaseMap.insert(std::make_pair(newLocBti, ConstantPointerNull::get(
                            cast<PointerType>(pointerOp->getType()))));

                    BtiValueMap.insert(std::make_pair(newLocBase, ConstantInt::get(Type::getInt32Ty(ptr->getContext()),
                                                                                   BTI_PRIVATE)));
                    pointerBaseMap.insert(std::make_pair(newLocBase, ConstantPointerNull::get(
                            cast<PointerType>(pointerOp->getType()))));
    ...
    void GenWriter::handleStoreLoadAddress(Function &F) {
        ...
        for (std::set<Value *>::iterator iter = addrStoreInst.begin(); iter != addrStoreInst.end(); ++iter) {
            ...
            Value *pointerBaseArray = Builder.CreateAlloca(AITy->getElementType(), ArraySize,
                                                           base->getName() + ".pointer-base");
    ...
    void
    GenWriter::getConstantData(const Constant *c, void *mem, uint32_t &offset, vector<ir::RelocEntry> &relocs) const {
        Type *type = c->getType();
...
#define GET_EFFECT_DATA(_seq, _index, _tid) \
    ...
    ir::ImmediateIndex GenWriter::processConstantImmIndexImpl(Constant *CPV, int32_t index) {
        ...
        } else if (dyn_cast<ConstantAggregateZero>(CPV)) {
            Type *Ty = CPV->getType();
            if (Ty->isVectorTy())
        ...
        if (!F.arg_empty()) {
            ...
            for (; I != E; ++I, ++argID) {
...
#else
                    llvmInfo.addrSpace = (mdconst::extract<ConstantInt>(
                            addrSpaceNode->getOperand(opID)))->getZExtValue();
#endif
                }
                if (typeNameNode) {
                    llvmInfo.typeName = (cast<MDString>(typeNameNode->getOperand(opID)))->getString();
                ...
                if (type->isVectorTy()) {
                    ...
                    ctx.input(argName, ir::FunctionArgument::VALUE, reg, llvmInfo, getTypeByteSize(unit, type),
                              getAlignmentByte(unit, type), 0);
                ...
                if (llvmInfo.isPipeType()) {
                    ...
                    ctx.input(argName, ir::FunctionArgument::PIPE, reg, llvmInfo, getTypeByteSize(unit, type),
                              getAlignmentByte(unit, type), BtiMap.find(&*I)->second);
                    continue;
                }
                ...
                if (type->isPointerTy() == false)
                    ...
                    if (I->hasByValAttr()) {
                        ...
                        ctx.input(argName, ir::FunctionArgument::STRUCTURE, reg, llvmInfo, structSize,
                                  getAlignmentByte(unit, type), 0);
                    }
                        ...
                        switch (addrSpace) {
                            ...
                            case ir::MEM_CONSTANT:
                                ctx.input(argName, ir::FunctionArgument::CONSTANT_POINTER, reg, llvmInfo, ptrSize,
                                          align, 0x2);
                                break;
                            default:
                                GBE_ASSERT(addrSpace != ir::MEM_PRIVATE);
                        }
                    }
                }
            }
    ...
    __attribute__((unused)) static inline bool isFPIntBitCast(const Instruction &I) {
...
*/
    ...
    void GenWriter::emitBinaryOperator(Instruction &I) {
        ...
        switch (I.getOpcode()) {
            ...
            case Instruction::FAdd:
                ctx.ADD(type, dst, src0, src1);
                break;
            case Instruction::Sub:
            case Instruction::FSub:
                ctx.SUB(type, dst, src0, src1);
                break;
            ...
            case Instruction::URem:
                ctx.REM(getUnsignedType(ctx, I.getType()), dst, src0, src1);
                break;
            case Instruction::SRem:
            case Instruction::FRem:
                ctx.REM(type, dst, src0, src1);
                break;
            ...
            case Instruction::FDiv:
                ctx.DIV(type, dst, src0, src1);
                break;
            case Instruction::And:
                ctx.AND(type, dst, src0, src1);
                break;
            case Instruction::Or:
                ctx.OR(type, dst, src0, src1);
                break;
            case Instruction::Xor:
                ctx.XOR(type, dst, src0, src1);
                break;
            case Instruction::Shl:
                ctx.SHL(type, dst, src0, src1);
                break;
            case Instruction::LShr:
                ctx.SHR(getUnsignedType(ctx, I.getType()), dst, src0, src1);
                break;
            case Instruction::AShr:
                ctx.ASR(type, dst, src0, src1);
                break;
            default:
                NOT_SUPPORTED;
        }
        ...
        if (conditionSet.find(&I) != conditionSet.end()) {
            switch (I.getPredicate()) {
                ...
                case ICmpInst::ICMP_ULE:
                    ctx.GT((unsignedType), dst, src0, src1);
                    break;
                case ICmpInst::ICMP_SLE:
                    ctx.GT(signedType, dst, src0, src1);
                    break;
                case ICmpInst::ICMP_UGE:
                    ctx.LT(unsignedType, dst, src0, src1);
                    break;
                case ICmpInst::ICMP_SGE:
                    ctx.LT(signedType, dst, src0, src1);
                    break;
                case ICmpInst::ICMP_ULT:
                    ctx.GE(unsignedType, dst, src0, src1);
                    break;
                case ICmpInst::ICMP_SLT:
                    ctx.GE(signedType, dst, src0, src1);
                    break;
                case ICmpInst::ICMP_UGT:
                    ctx.LE(unsignedType, dst, src0, src1);
                    break;
                case ICmpInst::ICMP_SGT:
                    ctx.LE(signedType, dst, src0, src1);
                    break;
                default:
                    NOT_SUPPORTED;
            }
   ...
   *  load/store, so keep empty function here */
    ...
    void GenWriter::regAllocateCallInst(CallInst &I) {
        ...
        switch (genIntrinsicID) {
            ...
            case GEN_OCL_GET_ENQUEUED_LOCAL_SIZE0:
                regTranslator.newScalarProxy(ir::ocl::enqlsize0, dst);
                break;
            case GEN_OCL_GET_ENQUEUED_LOCAL_SIZE1:
                regTranslator.newScalarProxy(ir::ocl::enqlsize1, dst);
                break;
            case GEN_OCL_GET_ENQUEUED_LOCAL_SIZE2:
                regTranslator.newScalarProxy(ir::ocl::enqlsize2, dst);
                break;
            ...
            case GEN_OCL_GET_THREAD_NUM:
                regTranslator.newScalarProxy(ir::ocl::threadn, dst);
                break;
            case GEN_OCL_GET_THREAD_ID:
                regTranslator.newScalarProxy(ir::ocl::threadid, dst);
                break;
            case GEN_OCL_GET_WORK_DIM:
                regTranslator.newScalarProxy(ir::ocl::workdim, dst);
                break;
            case GEN_OCL_FBH:
            case GEN_OCL_FBL:
            case GEN_OCL_CBIT:
    ...
    void GenWriter::emitAtomicInstHelper(const ir::AtomicOps opcode, const ir::Type type, const ir::Register dst,
                                         llvm::Value *llvmPtr, const ir::Tuple payloadTuple) {
    ...
    static INLINE ir::AtomicOps atomicOpsLLVMToGen(llvm::AtomicRMWInst::BinOp llvmOp) {
        switch (llvmOp) {
            case llvm::AtomicRMWInst::Xchg:
                return ir::ATOMIC_OP_XCHG;
            case llvm::AtomicRMWInst::Add:
                return ir::ATOMIC_OP_ADD;
            case llvm::AtomicRMWInst::Sub:
                return ir::ATOMIC_OP_SUB;
            case llvm::AtomicRMWInst::And:
                return ir::ATOMIC_OP_AND;
            case llvm::AtomicRMWInst::Or:
                return ir::ATOMIC_OP_OR;
            case llvm::AtomicRMWInst::Xor:
                return ir::ATOMIC_OP_XOR;
            case llvm::AtomicRMWInst::Max:
                return ir::ATOMIC_OP_IMAX;
            case llvm::AtomicRMWInst::Min:
                return ir::ATOMIC_OP_IMIN;
            case llvm::AtomicRMWInst::UMax:
                return ir::ATOMIC_OP_UMAX;
            case llvm::AtomicRMWInst::UMin:
                return ir::ATOMIC_OP_UMIN;
                // FIXME: enumeration values 'FAdd' and 'FSub' not handled in switch
    ...
    void GenWriter::emitWorkGroupInst(CallInst &I, CallSite &CS, ir::WorkGroupOps opcode) {
       ...
       * Threads will use SLM to write partial results computed individually
        ...
        if (opcode == ir::WORKGROUP_OP_ALL || opcode == ir::WORKGROUP_OP_ANY) {
            ...
            ctx.WORKGROUP(ir::WORKGROUP_OP_BROADCAST, (uint32_t) f.getwgBroadcastSLM(), getRegister(&I), srcTuple,
                          argNum,
    ...
    void GenWriter::checkMediaBlockWidthandHeight(CallInst &I, uint8_t width, uint8_t height, uint8_t vec_size,
                                                  ir::Type type) {
        if (width == 0) {
            has_errors = true;
            Func->getContext().emitError(&I, "Media Block width value illegal, width is: " + std::to_string(width));
            ctx.getUnit().setValid(false);
            return;
        }
        if (height == 0) {
            has_errors = true;
            Func->getContext().emitError(&I, "Media Block height value illegal, height is: " + std::to_string(height));
            ctx.getUnit().setValid(false);
            return;
        }
        uint32_t typeSize;
        ...
        if (widthBytes % 4 != 0) {
            has_errors = true;
            Func->getContext().emitError(&I, "Media Block widthBytes value illegal, widthBytes is: " +
        ...
        if (height > maxRows) {
            has_errors = true;
            Func->getContext().emitError(&I, "Media Block height value illegal, height is larger than: " +
    ...
    GenWriter::emitBlockReadWriteImageInst(CallInst &I, CallSite &CS, bool isWrite, uint8_t vec_size, ir::Type type) {
        ...
        if (isWrite) {
            ...
            ctx.MBREAD(imageID, dsttuple, vec_size, srctuple, 2, type, width.getIntegerValue(),
                       height.getIntegerValue());
        }
            ...
            if (F->getIntrinsicID() != 0) {
                ...
                switch (F->getIntrinsicID()) {
                    ...
                    case Intrinsic::fmuladd: {
                        ...
                        break;
                    case Intrinsic::sqrt:
                        this->emitUnaryCallInst(I, CS, ir::OP_SQR);
                        break;
                    case Intrinsic::ceil:
                        this->emitRoundingCallInst(I, CS, ir::OP_RNDU);
                        break;
                    case Intrinsic::trunc:
                        this->emitRoundingCallInst(I, CS, ir::OP_RNDZ);
                        break;
                    case Intrinsic::rint:
                        this->emitRoundingCallInst(I, CS, ir::OP_RNDE);
                        break;
                    case Intrinsic::floor:
                        this->emitRoundingCallInst(I, CS, ir::OP_RNDD);
                        break;
                    case Intrinsic::sin:
                        this->emitUnaryCallInst(I, CS, ir::OP_SIN);
                        break;
                    case Intrinsic::cos:
                        this->emitUnaryCallInst(I, CS, ir::OP_COS);
                        break;
                    case Intrinsic::log2:
                        this->emitUnaryCallInst(I, CS, ir::OP_LOG);
                        break;
                    case Intrinsic::exp2:
                        this->emitUnaryCallInst(I, CS, ir::OP_EXP);
                        break;
                    case Intrinsic::bswap:
                        this->emitUnaryCallInst(I, CS, ir::OP_BSWAP, getUnsignedType(ctx, I.getType()));
                        break;
                    case Intrinsic::pow: {
                ...
                switch (genIntrinsicID) {
                    case GEN_OCL_FBH:
                        this->emitUnaryCallInst(I, CS, ir::OP_FBH, ir::TYPE_U32);
                        break;
                    case GEN_OCL_FBL:
                        this->emitUnaryCallInst(I, CS, ir::OP_FBL, ir::TYPE_U32);
                        break;
                    case GEN_OCL_CBIT:
                        this->emitUnaryCallInst(I, CS, ir::OP_CBIT, getUnsignedType(ctx, (*AI)->getType()));
                        break;
                    case GEN_OCL_ABS: {
                        const ir::Register src = this->getRegister(*AI);
                        const ir::Register dst = this->getRegister(&I);
                    ...
                    case GEN_OCL_RSQ:
                        this->emitUnaryCallInst(I, CS, ir::OP_RSQ);
                        break;
                    case GEN_OCL_RCP:
                        this->emitUnaryCallInst(I, CS, ir::OP_RCP);
                        break;
                    case GEN_OCL_FORCE_SIMD8:
                        ctx.setSimdWidth(8);
                        break;
                    ...
                    case GEN_OCL_ATOMIC_ADD0:
                    case GEN_OCL_ATOMIC_ADD1:
                        this->emitAtomicInst(I, CS, ir::ATOMIC_OP_ADD);
                        break;
                    case GEN_OCL_ATOMIC_SUB0:
                    case GEN_OCL_ATOMIC_SUB1:
                        this->emitAtomicInst(I, CS, ir::ATOMIC_OP_SUB);
                        break;
                    case GEN_OCL_ATOMIC_AND0:
                    case GEN_OCL_ATOMIC_AND1:
                        this->emitAtomicInst(I, CS, ir::ATOMIC_OP_AND);
                        break;
                    case GEN_OCL_ATOMIC_OR0:
                    case GEN_OCL_ATOMIC_OR1:
                        this->emitAtomicInst(I, CS, ir::ATOMIC_OP_OR);
                        break;
                    case GEN_OCL_ATOMIC_XOR0:
                    case GEN_OCL_ATOMIC_XOR1:
                        this->emitAtomicInst(I, CS, ir::ATOMIC_OP_XOR);
                        break;
                    case GEN_OCL_ATOMIC_XCHG0:
                    case GEN_OCL_ATOMIC_XCHG1:
                        this->emitAtomicInst(I, CS, ir::ATOMIC_OP_XCHG);
                        break;
                    case GEN_OCL_ATOMIC_INC0:
                    case GEN_OCL_ATOMIC_INC1:
                        this->emitAtomicInst(I, CS, ir::ATOMIC_OP_INC);
                        break;
                    case GEN_OCL_ATOMIC_DEC0:
                    case GEN_OCL_ATOMIC_DEC1:
                        this->emitAtomicInst(I, CS, ir::ATOMIC_OP_DEC);
                        break;
                    case GEN_OCL_ATOMIC_UMIN0:
                    case GEN_OCL_ATOMIC_UMIN1:
                        this->emitAtomicInst(I, CS, ir::ATOMIC_OP_UMIN);
                        break;
                    case GEN_OCL_ATOMIC_UMAX0:
                    case GEN_OCL_ATOMIC_UMAX1:
                    ...
                    case GEN_OCL_ATOMIC_IMIN0:
                    case GEN_OCL_ATOMIC_IMIN1:
                        this->emitAtomicInst(I, CS, ir::ATOMIC_OP_IMIN);
                        break;
                    case GEN_OCL_ATOMIC_IMAX0:
                    case GEN_OCL_ATOMIC_IMAX1:
                        this->emitAtomicInst(I, CS, ir::ATOMIC_OP_IMAX);
                        break;
                    case GEN_OCL_ATOMIC_CMPXCHG0:
                    case GEN_OCL_ATOMIC_CMPXCHG1:
                        this->emitAtomicInst(I, CS, ir::ATOMIC_OP_CMPXCHG);
                        break;
                    case GEN_OCL_GET_IMAGE_WIDTH:
                    case GEN_OCL_GET_IMAGE_HEIGHT:
                    case GEN_OCL_GET_IMAGE_DEPTH:
...
#define DEF(DST_TYPE, SRC_TYPE) \
  { ctx.SAT_CVT(DST_TYPE, SRC_TYPE, getRegister(&I), getRegister(I.getOperand(0))); break; }
                    case GEN_OCL_SAT_CONV_U8_TO_I8: DEF(ir::TYPE_S8, ir::TYPE_U8);
                    case GEN_OCL_SAT_CONV_I16_TO_I8: DEF(ir::TYPE_S8, ir::TYPE_S16);
                    case GEN_OCL_SAT_CONV_U16_TO_I8: DEF(ir::TYPE_S8, ir::TYPE_U16);
                    case GEN_OCL_SAT_CONV_I32_TO_I8: DEF(ir::TYPE_S8, ir::TYPE_S32);
                    case GEN_OCL_SAT_CONV_U32_TO_I8: DEF(ir::TYPE_S8, ir::TYPE_U32);
                    case GEN_OCL_SAT_CONV_F32_TO_I8: DEF(ir::TYPE_S8, ir::TYPE_FLOAT);
                    case GEN_OCL_SAT_CONV_I8_TO_U8: DEF(ir::TYPE_U8, ir::TYPE_S8);
                    case GEN_OCL_SAT_CONV_I16_TO_U8: DEF(ir::TYPE_U8, ir::TYPE_S16);
                    case GEN_OCL_SAT_CONV_U16_TO_U8: DEF(ir::TYPE_U8, ir::TYPE_U16);
                    case GEN_OCL_SAT_CONV_I32_TO_U8: DEF(ir::TYPE_U8, ir::TYPE_S32);
                    case GEN_OCL_SAT_CONV_U32_TO_U8: DEF(ir::TYPE_U8, ir::TYPE_U32);
                    case GEN_OCL_SAT_CONV_F32_TO_U8: DEF(ir::TYPE_U8, ir::TYPE_FLOAT);
                    case GEN_OCL_SAT_CONV_U16_TO_I16: DEF(ir::TYPE_S16, ir::TYPE_U16);
                    case GEN_OCL_SAT_CONV_I32_TO_I16: DEF(ir::TYPE_S16, ir::TYPE_S32);
                    case GEN_OCL_SAT_CONV_U32_TO_I16: DEF(ir::TYPE_S16, ir::TYPE_U32);
                    case GEN_OCL_SAT_CONV_F32_TO_I16: DEF(ir::TYPE_S16, ir::TYPE_FLOAT);
                    case GEN_OCL_SAT_CONV_F64_TO_I16: DEF(ir::TYPE_S16, ir::TYPE_DOUBLE);
                    case GEN_OCL_SAT_CONV_I16_TO_U16: DEF(ir::TYPE_U16, ir::TYPE_S16);
                    case GEN_OCL_SAT_CONV_I32_TO_U16: DEF(ir::TYPE_U16, ir::TYPE_S32);
                    case GEN_OCL_SAT_CONV_U32_TO_U16: DEF(ir::TYPE_U16, ir::TYPE_U32);
                    case GEN_OCL_SAT_CONV_F32_TO_U16: DEF(ir::TYPE_U16, ir::TYPE_FLOAT);
                    case GEN_OCL_SAT_CONV_F64_TO_U16: DEF(ir::TYPE_U16, ir::TYPE_DOUBLE);
                    case GEN_OCL_SAT_CONV_U32_TO_I32: DEF(ir::TYPE_S32, ir::TYPE_U32);
                    case GEN_OCL_SAT_CONV_F32_TO_I32: DEF(ir::TYPE_S32, ir::TYPE_FLOAT);
                    case GEN_OCL_SAT_CONV_F64_TO_I32: DEF(ir::TYPE_S32, ir::TYPE_DOUBLE);
                    case GEN_OCL_SAT_CONV_I32_TO_U32: DEF(ir::TYPE_U32, ir::TYPE_S32);
                    case GEN_OCL_SAT_CONV_F32_TO_U32: DEF(ir::TYPE_U32, ir::TYPE_FLOAT);
                    case GEN_OCL_SAT_CONV_F64_TO_U32: DEF(ir::TYPE_U32, ir::TYPE_DOUBLE);
                    case GEN_OCL_SAT_CONV_F16_TO_I8: DEF(ir::TYPE_S8, ir::TYPE_HALF);
                    case GEN_OCL_SAT_CONV_F16_TO_U8: DEF(ir::TYPE_U8, ir::TYPE_HALF);
                    case GEN_OCL_SAT_CONV_F16_TO_I16: DEF(ir::TYPE_S16, ir::TYPE_HALF);
                    case GEN_OCL_SAT_CONV_F16_TO_U16: DEF(ir::TYPE_U16, ir::TYPE_HALF);
                    case GEN_OCL_SAT_CONV_F16_TO_I32: DEF(ir::TYPE_S32, ir::TYPE_HALF);
                    case GEN_OCL_SAT_CONV_F16_TO_U32: DEF(ir::TYPE_U32, ir::TYPE_HALF);
                    case GEN_OCL_CONV_F16_TO_F32:
                        ctx.F16TO32(ir::TYPE_FLOAT, ir::TYPE_U16, getRegister(&I), getRegister(I.getOperand(0)));
                        break;
                    case GEN_OCL_CONV_F32_TO_F16:
...
#undef DEF
               ...
               We just pass down the BTI value to the instruction. */
                    ...
                    case GEN_OCL_WORK_GROUP_ALL:
                        this->emitWorkGroupInst(I, CS, ir::WORKGROUP_OP_ALL);
                        break;
                    case GEN_OCL_WORK_GROUP_ANY:
                        this->emitWorkGroupInst(I, CS, ir::WORKGROUP_OP_ANY);
                        break;
                    case GEN_OCL_WORK_GROUP_BROADCAST:
                        this->emitWorkGroupInst(I, CS, ir::WORKGROUP_OP_BROADCAST);
                        break;
                    case GEN_OCL_WORK_GROUP_REDUCE_ADD:
                        this->emitWorkGroupInst(I, CS, ir::WORKGROUP_OP_REDUCE_ADD);
                        break;
                    case GEN_OCL_WORK_GROUP_REDUCE_MAX:
                        this->emitWorkGroupInst(I, CS, ir::WORKGROUP_OP_REDUCE_MAX);
                        break;
                    case GEN_OCL_WORK_GROUP_REDUCE_MIN:
                        this->emitWorkGroupInst(I, CS, ir::WORKGROUP_OP_REDUCE_MIN);
                        break;
                    case GEN_OCL_WORK_GROUP_SCAN_EXCLUSIVE_ADD:
                        this->emitWorkGroupInst(I, CS, ir::WORKGROUP_OP_EXCLUSIVE_ADD);
                        break;
                    case GEN_OCL_WORK_GROUP_SCAN_EXCLUSIVE_MAX:
                        this->emitWorkGroupInst(I, CS, ir::WORKGROUP_OP_EXCLUSIVE_MAX);
                        break;
                    case GEN_OCL_WORK_GROUP_SCAN_EXCLUSIVE_MIN:
                        this->emitWorkGroupInst(I, CS, ir::WORKGROUP_OP_EXCLUSIVE_MIN);
                        break;
                    case GEN_OCL_WORK_GROUP_SCAN_INCLUSIVE_ADD:
                        this->emitWorkGroupInst(I, CS, ir::WORKGROUP_OP_INCLUSIVE_ADD);
                        break;
                    case GEN_OCL_WORK_GROUP_SCAN_INCLUSIVE_MAX:
                        this->emitWorkGroupInst(I, CS, ir::WORKGROUP_OP_INCLUSIVE_MAX);
                        break;
                    case GEN_OCL_WORK_GROUP_SCAN_INCLUSIVE_MIN:
                        this->emitWorkGroupInst(I, CS, ir::WORKGROUP_OP_INCLUSIVE_MIN);
                        break;
                    case GEN_OCL_SUB_GROUP_BROADCAST:
                        this->emitSubGroupInst(I, CS, ir::WORKGROUP_OP_BROADCAST);
                        break;
                    case GEN_OCL_SUB_GROUP_REDUCE_ADD:
                        this->emitSubGroupInst(I, CS, ir::WORKGROUP_OP_REDUCE_ADD);
                        break;
                    case GEN_OCL_SUB_GROUP_REDUCE_MAX:
                        this->emitSubGroupInst(I, CS, ir::WORKGROUP_OP_REDUCE_MAX);
                        break;
                    case GEN_OCL_SUB_GROUP_REDUCE_MIN:
                        this->emitSubGroupInst(I, CS, ir::WORKGROUP_OP_REDUCE_MIN);
                        break;
                    case GEN_OCL_SUB_GROUP_SCAN_EXCLUSIVE_ADD:
                        this->emitSubGroupInst(I, CS, ir::WORKGROUP_OP_EXCLUSIVE_ADD);
                        break;
                    case GEN_OCL_SUB_GROUP_SCAN_EXCLUSIVE_MAX:
                        this->emitSubGroupInst(I, CS, ir::WORKGROUP_OP_EXCLUSIVE_MAX);
                        break;
                    case GEN_OCL_SUB_GROUP_SCAN_EXCLUSIVE_MIN:
                        this->emitSubGroupInst(I, CS, ir::WORKGROUP_OP_EXCLUSIVE_MIN);
                        break;
                    case GEN_OCL_SUB_GROUP_SCAN_INCLUSIVE_ADD:
                        this->emitSubGroupInst(I, CS, ir::WORKGROUP_OP_INCLUSIVE_ADD);
                        break;
                    case GEN_OCL_SUB_GROUP_SCAN_INCLUSIVE_MAX:
                        this->emitSubGroupInst(I, CS, ir::WORKGROUP_OP_INCLUSIVE_MAX);
                        break;
                    case GEN_OCL_SUB_GROUP_SCAN_INCLUSIVE_MIN:
                        this->emitSubGroupInst(I, CS, ir::WORKGROUP_OP_INCLUSIVE_MIN);
                        break;
                    case GEN_OCL_LRP: {
                        const ir::Register dst = this->getRegister(&I);
                        GBE_ASSERT(AI != AE);
                    ...
                    case GEN_OCL_SUB_GROUP_BLOCK_READ_UI_MEM:
                        this->emitBlockReadWriteMemInst(I, CS, false, 1);
                        break;
                    case GEN_OCL_SUB_GROUP_BLOCK_READ_UI_MEM2:
                        this->emitBlockReadWriteMemInst(I, CS, false, 2);
                        break;
                    case GEN_OCL_SUB_GROUP_BLOCK_READ_UI_MEM4:
                        this->emitBlockReadWriteMemInst(I, CS, false, 4);
                        break;
                    case GEN_OCL_SUB_GROUP_BLOCK_READ_UI_MEM8:
                        this->emitBlockReadWriteMemInst(I, CS, false, 8);
                        break;
                    case GEN_OCL_SUB_GROUP_BLOCK_WRITE_UI_MEM:
                        this->emitBlockReadWriteMemInst(I, CS, true, 1);
                        break;
                    case GEN_OCL_SUB_GROUP_BLOCK_WRITE_UI_MEM2:
                        this->emitBlockReadWriteMemInst(I, CS, true, 2);
                        break;
                    case GEN_OCL_SUB_GROUP_BLOCK_WRITE_UI_MEM4:
                        this->emitBlockReadWriteMemInst(I, CS, true, 4);
                        break;
                    case GEN_OCL_SUB_GROUP_BLOCK_WRITE_UI_MEM8:
                        this->emitBlockReadWriteMemInst(I, CS, true, 8);
                        break;
                    case GEN_OCL_SUB_GROUP_BLOCK_READ_UI_IMAGE:
                        this->emitBlockReadWriteImageInst(I, CS, false, 1);
                        break;
                    case GEN_OCL_SUB_GROUP_BLOCK_READ_UI_IMAGE2:
                        this->emitBlockReadWriteImageInst(I, CS, false, 2);
                        break;
                    case GEN_OCL_SUB_GROUP_BLOCK_READ_UI_IMAGE4:
                        this->emitBlockReadWriteImageInst(I, CS, false, 4);
                        break;
                    case GEN_OCL_SUB_GROUP_BLOCK_READ_UI_IMAGE8:
                        this->emitBlockReadWriteImageInst(I, CS, false, 8);
                        break;
                    case GEN_OCL_SUB_GROUP_BLOCK_WRITE_UI_IMAGE:
                        this->emitBlockReadWriteImageInst(I, CS, true, 1);
                        break;
                    case GEN_OCL_SUB_GROUP_BLOCK_WRITE_UI_IMAGE2:
                        this->emitBlockReadWriteImageInst(I, CS, true, 2);
                        break;
                    case GEN_OCL_SUB_GROUP_BLOCK_WRITE_UI_IMAGE4:
                        this->emitBlockReadWriteImageInst(I, CS, true, 4);
                        break;
                    case GEN_OCL_SUB_GROUP_BLOCK_WRITE_UI_IMAGE8:
                        this->emitBlockReadWriteImageInst(I, CS, true, 8);
                        break;
                    case GEN_OCL_SUB_GROUP_BLOCK_READ_US_MEM:
                    ...
                    case GEN_OCL_SUB_GROUP_BLOCK_READ_US_IMAGE8:
                        ...
                        break;
                    case GEN_OCL_SUB_GROUP_BLOCK_READ_US_IMAGE16:
                        this->emitBlockReadWriteImageInst(I, CS, false, 16, ir::TYPE_U16);
                        break;
                    case GEN_OCL_SUB_GROUP_BLOCK_READ_UC_IMAGE:
                        this->emitBlockReadWriteImageInst(I, CS, false, 1, ir::TYPE_U8);
                        break;
                    case GEN_OCL_SUB_GROUP_BLOCK_READ_UC_IMAGE2:
                        this->emitBlockReadWriteImageInst(I, CS, false, 2, ir::TYPE_U8);
                        break;
                    case GEN_OCL_SUB_GROUP_BLOCK_READ_UC_IMAGE4:
                        this->emitBlockReadWriteImageInst(I, CS, false, 4, ir::TYPE_U8);
                        break;
                    case GEN_OCL_SUB_GROUP_BLOCK_READ_UC_IMAGE8:
                        this->emitBlockReadWriteImageInst(I, CS, false, 8, ir::TYPE_U8);
                        break;
                    case GEN_OCL_SUB_GROUP_BLOCK_READ_UC_IMAGE16:
                        this->emitBlockReadWriteImageInst(I, CS, false, 16, ir::TYPE_U8);
                        break;
                    case GEN_OCL_SUB_GROUP_BLOCK_WRITE_US_IMAGE:
                    ...
                    case GEN_OCL_SUB_GROUP_BLOCK_WRITE_US_IMAGE8:
                        ...
                        break;
                    case GEN_OCL_SUB_GROUP_BLOCK_WRITE_US_IMAGE16:
                        this->emitBlockReadWriteImageInst(I, CS, true, 16, ir::TYPE_U16);
                        break;
                    case GEN_OCL_SUB_GROUP_BLOCK_WRITE_UC_IMAGE:
                        this->emitBlockReadWriteImageInst(I, CS, true, 1, ir::TYPE_U8);
                        break;
                    case GEN_OCL_SUB_GROUP_BLOCK_WRITE_UC_IMAGE2:
                        this->emitBlockReadWriteImageInst(I, CS, true, 2, ir::TYPE_U8);
                        break;
                    case GEN_OCL_SUB_GROUP_BLOCK_WRITE_UC_IMAGE4:
                        this->emitBlockReadWriteImageInst(I, CS, true, 4, ir::TYPE_U8);
                        break;
                    case GEN_OCL_SUB_GROUP_BLOCK_WRITE_UC_IMAGE8:
                        this->emitBlockReadWriteImageInst(I, CS, true, 8, ir::TYPE_U8);
                        break;
                    case GEN_OCL_SUB_GROUP_BLOCK_WRITE_UC_IMAGE16:
                        this->emitBlockReadWriteImageInst(I, CS, true, 16, ir::TYPE_U8);
                        break;
                    case GEN_OCL_GET_PIPE:
                    case GEN_OCL_MAKE_RID:
                    case GEN_OCL_GET_RID:
    ...
    ir::Tuple
    MemoryInstHelper::getValueTuple(llvm::Value *llvmValues, llvm::Type *elemType, unsigned start, unsigned elemNum) {
