...
#undef DECL_INSN
            ...
            struct NSrcPolicy {
                INLINE uint32_t getSrcNum() const { return srcNum; }
            ...
            struct NDstPolicy {
                INLINE uint32_t getDstNum() const { return dstNum; }
            ...
            public:
                SampleInstruction(uint8_t imageIdx, Tuple dstTuple, Tuple srcTuple, uint8_t srcNum, bool dstIsFloat,
                                  bool srcIsFloat, uint8_t sampler, uint8_t samplerOffset) {
            ...
            public:
                ...
                INLINE TypedWriteInstruction(uint8_t imageIdx, Tuple srcTuple, uint8_t srcNum, Type srcType,
                                             Type coordType) {
            ...
            public:
                INLINE MediaBlockReadInstruction(uint8_t imageIdx, Tuple dst, uint8_t vec_size, Tuple srcTuple,
                                                 uint8_t srcNum, Type type, uint8_t width, uint8_t height) {
            ...
            public:
                ...
                INLINE MediaBlockWriteInstruction(uint8_t imageIdx, Tuple srcTuple, uint8_t srcNum, uint8_t vec_size,
                                                  Type type, uint8_t width, uint8_t height) {
  ...
  do { \
    if (UNLIKELY(checkTypeFamily(TYPE, FAMILY, FAMILY##Num, whyNot)) == false) \
      return false; \
            ...
            // TODO
            INLINE bool SampleInstruction::wellFormed(const Function &fn, std::string &why) const { return true; }

            INLINE bool VmeInstruction::wellFormed(const Function &fn, std::string &why) const { return true; }

            INLINE bool TypedWriteInstruction::wellFormed(const Function &fn, std::string &why) const { return true; }

            INLINE bool GetImageInfoInstruction::wellFormed(const Function &fn, std::string &why) const { return true; }

            INLINE bool WaitInstruction::wellFormed(const Function &fn, std::string &why) const { return true; }
...
#define OUT_ATOMIC_OP(TYPE)     \
      ...
      {    out << "." << #TYPE; \
        ...
        struct HelperIntrospection {
            enum {
                value = 0
            };
        };
        template<typename T>
        struct HelperIntrospection<T, T> {
            enum {
                value = 1
            };
        };
...
#undef DECL_EMIT_FUNCTION
        ...
        // For all unary functions with given opcode
        Instruction
        ATOMIC(AtomicOps atomicOp, Type type, Register dst, AddressSpace space, Register address, Tuple payload,
               AddressMode AM, Register bti) {
            internal::AtomicInstruction insn = internal::AtomicInstruction(atomicOp, type, dst, space, address, payload,
                                                                           AM);
            insn.setBtiReg(bti);
            return insn.convert();
        }
        ...
        Instruction
        ATOMIC(AtomicOps atomicOp, Type type, Register dst, AddressSpace space, Register address, Tuple payload,
               AddressMode AM, unsigned SurfaceIndex) {
            internal::AtomicInstruction insn = internal::AtomicInstruction(atomicOp, type, dst, space, address, payload,
                                                                           AM);
...
#undef DECL_EMIT_FUNCTION
        ...
        // SAMPLE
        Instruction SAMPLE(uint8_t imageIndex, Tuple dst, Tuple src, uint8_t srcNum, bool dstIsFloat, bool srcIsFloat,
                           uint8_t sampler, uint8_t samplerOffset) {
            return internal::SampleInstruction(imageIndex, dst, src, srcNum, dstIsFloat, srcIsFloat, sampler,
                                               samplerOffset).convert();
        }

        Instruction VME(uint8_t imageIndex, Tuple dst, Tuple src, uint32_t dstNum, uint32_t srcNum, int msg_type,
                        int vme_search_path_lut, int lut_sub) {
            return internal::VmeInstruction(imageIndex, dst, src, dstNum, srcNum, msg_type, vme_search_path_lut,
                                            lut_sub).convert();
        }
        ...
        Instruction
        WORKGROUP(WorkGroupOps opcode, uint32_t slmAddr, Register dst, Tuple srcTuple, uint8_t srcNum, Type type) {
        ...
        Instruction
        MBREAD(uint8_t imageIndex, Tuple dst, uint8_t vec_size, Tuple coord, uint8_t srcNum, Type type, uint8_t width,
               uint8_t height) {
            return internal::MediaBlockReadInstruction(imageIndex, dst, vec_size, coord, srcNum, type, width,
                                                       height).convert();
        }

        Instruction
        MBWRITE(uint8_t imageIndex, Tuple srcTuple, uint8_t srcNum, uint8_t vec_size, Type type, uint8_t width,
                uint8_t height) {
            return internal::MediaBlockWriteInstruction(imageIndex, srcTuple, srcNum, vec_size, type, width,
                                                        height).convert();
        }
