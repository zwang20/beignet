    ...
    namespace ir {
        ...
        void Function::outImmediate(std::ostream &out, ImmediateIndex index) const {
            ...
            switch (imm.getType()) {
                ...
                case TYPE_S64:
                    ...
                    break;
                case TYPE_U64:
                    out << (uint64_t) imm.getIntegerValue();
                    break;
                case TYPE_HALF:
                    out << "half(" << (float) imm.getHalfValue() << ")";
                    break;
                case TYPE_FLOAT:
                    out << imm.getFloatValue();
                    break;
                case TYPE_DOUBLE:
                    out << imm.getDoubleValue();
                    break;
                default:
        ...
        void Function::computeCFG(void) {
            ...
            this->foreachBlock([this, &jumpToNext](BasicBlock &bb) {
                ...
                if (last->isMemberOf<BranchInstruction>() == false || last->getOpcode() == OP_ENDIF ||
                    last->getOpcode() == OP_ELSE) {
                    jumpToNext = &bb;
                    return;
                }
        ...
        std::ostream &operator<<(std::ostream &out, const Function &fn) {
            ...
            for (uint32_t i = 0; i < fn.argNum(); ++i) {
                ...
                out << "decl_input.";
                switch (input.type) {
                    case FunctionArgument::GLOBAL_POINTER:
                        out << "global";
                        break;
                    case FunctionArgument::LOCAL_POINTER:
                        out << "local";
                        break;
                    case FunctionArgument::CONSTANT_POINTER:
                        out << "constant";
                        break;
                    case FunctionArgument::VALUE:
                        out << "value";
                        break;
                    case FunctionArgument::STRUCTURE:
                        out << "structure." << input.size;
                        break;
                    case FunctionArgument::IMAGE:
                        out << "image";
                        break;
                    case FunctionArgument::PIPE:
                        out << "pipe";
                        break;
                    default:
                        break;
